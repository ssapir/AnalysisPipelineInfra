...
class VideoFromRaw:
    ...
    def __init__(self, fullname, start_frame=None, FRAME_ROWS=1024, FRAME_COLS=512, dtype=np.uint8):  # start_frame starts from 1
    ...
    def create_file(self):
        ...
        try:
            ...
            self.video = np.memmap(self.fullname, dtype=self.dtype, mode='r').reshape([-1, self.FRAME_COLS, self.FRAME_ROWS])
    ...
    def read(self):
        ...
        if self.isOpened() and self.curr_frame_ind < self.n_frames:  # num of frames is 3rd
            ...
            dtype = 'uint16' if self.dtype == np.int16 else 'uint8'
            return ok, cv2.cvtColor(frame.astype(dtype), cv2.COLOR_GRAY2BGR)
        else:
            print("Reached the end of ", self.video.shape)
            return False, np.array([])

    def readFrame(self, frame_ind):
        # release memory if needed before reading further
        frame = self.video[frame_ind, :, :]
        dtype = 'uint16' if self.dtype == np.int16 else 'uint8'
        return True, cv2.cvtColor(frame.astype('uint16'), cv2.COLOR_GRAY2BGR)
...
def open_raw(inputfolder, vidname, start_frame=None, FRAME_ROWS=1024, FRAME_COLS=512, dtype=np.uint8):
    fullname = os.path.join(inputfolder, vidname)
    video = VideoFromRaw(fullname, start_frame=start_frame, FRAME_ROWS=FRAME_ROWS, FRAME_COLS=FRAME_COLS, dtype=dtype)
...
def open(inputfolder, vidname, start_frame=None, FRAME_ROWS=1024, FRAME_COLS=512, dtype=np.uint8):
    if vidname.lower().endswith(".avi"):
        return open_avi(inputfolder, vidname, start_frame=start_frame)
    elif vidname.lower().endswith(".raw"):
        return open_raw(inputfolder, vidname, start_frame=start_frame, FRAME_ROWS=FRAME_ROWS, FRAME_COLS=FRAME_COLS, dtype=dtype)
    else:
